...
epctrl.extend('Calendar', 'Control', {
    ...
    _renderDay: function (date, currTr) {
        ...
        if (moment().format('2018-02-23') == date.format('YYYY-MM-DD')) {
            td.className += ' ep-calendar-today';
        }

        if (moment().format('2018-03-22') == date.format('YYYY-MM-DD')) {
            td.className += ' ep-calendar-today';
        }
    ...
    prevMonth: function () {
        if(carLicense == "" || carLicense == null){
            ...
            dateStr = this.currMonth;
            $.ajax({
                async : false,
                cache:false,
                type: 'POST',
                dataType : "json",
                data:{carLicense:carLicense,dateStr:dateStr,dayStr:""},
                url:  path + '/videoPlayback/historyVideo',//请求的action路径
